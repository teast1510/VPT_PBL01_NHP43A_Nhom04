    ...
    struct RentalRecord { // cấu trúc cho lịch sử thuê/ hủy
        string boatID;
        string ticketCode;
        float seatsRented;
        int shiftIndex;
    ...
    struct Shift {  // cấu trúc cho ca làm việc
        int startHour;
        int endHour;
        float pricePerHour;
        float seatsBooked;
    ...
    void pressEnterToContinue2() {  // cùng tác dụng nhưng dùng cho các đoạn bị lỗi nhập 2 lần
        cout << "\n\t\tNhan Enter de tiep tuc...";
        cin.get();
    }
    ...
    bool isValidNumber(const string& str) {     // Hàm hỗ trợ kiểm tra số hợp lệ
        try {
            size_t pos;
            stof(str, &pos); 
    ...
    public:
        ...
        virtual ~Boat() {}
        virtual void createBoat() = 0; // hàm tạo thuyền
        virtual void display() const = 0;
        char getType() const {
            return type;
        }
    
        bool getAvailability() const {// hàm trả về giá trị hoạt động
            return isAvailable;
        }                    
        float getSeats() const { // hàm trả về giá trị số ghế của thuyền
            return seats; 
        }                                
        string getBoatName() const {   // hàm trả về tên thuyền
            return boatName;
        }                         
        string getBoatID() const {   // hàm trẻ về ID thuyền
            return boatID;
        }                            
        int getRentalCount() const {     // hàm trả về số lần đã thuê
            return rentalCount; 
        }                     
        const vector<Shift>& getShifts() const {    // hàm trả về mảng các thông số của các ca trực của thuyền
            return shifts; 
        }               
        int getNumShifts() const {  // hàm trả về số ca trực  
            return numShifts; 
        }                          
        void setType(char t) {  // hàm đặt giá trị của loại thuyền
            type = t;
        }
        void setAvailability(bool available) {  // hàm đặt giá trị hoạt động 
    ...
    public:
        SmallBoat() { setType('S'); }
        void createBoat() override {    // đa hình, ghi đè cho hàm creaBoat
            system("cls");
            cout << "\n\t\t======================================";
            cout << "\n\t\t           TAO THUYEN NHO    ";
            cout << "\n\t\t======================================\n";
        ...
        void display() const override {            // áp dụng đa hình (ghi đè) cho 2 loại Small và Large
            ...
            for (int i = 0; i < numShifts; i++) {
                cout << "\t\tCa " << i + 1 << ": " << shifts[i].startHour << "h-"
                    ...
                    << " VND/gio, so ghe da dat: " << shifts[i].seatsBooked << "\n";
            }
    ...
    public:
        ...
        void createBoat() override {
            ...
            do {
                cout << "\t\tNhap so ghe (toi thieu 101): ";
                cin >> input;
    ...
    class BoatManager {
        ...
        const string password = "BKDaNang";
    ...
    public:
        ...
        void addBoat() {            
            ...
            char type;
            do {
                cout << "\n\t\tVui Long nhap mat khau may chu de them thuyen moi: ";
                string pass;
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                getline(cin, pass);
                if (pass != password) {
        ...
        void rentSeats() {  // hàm mua vé
            ...
            for (int i = 0; i < boat->getNumShifts(); i++) {
                cout << "\t\tCa " << i + 1 << ": " << shifts[i].startHour << "h-"
                    << shifts[i].endHour << "h, Gia:" << shifts[i].pricePerHour
                    << " VND/gio, Ghe con trong: " << boat->getSeats() - shifts[i].seatsBooked << "\n";
            }
            ...
            do {
                ...
                if (!isValidNumber(input) || stof(input) <= 0 ||
                    ...
                    cout << "\t\tChi phi thue:" << rentalCost << " VND\n";
                    break;
                }
            } while (true);
            ...
            if (orderFood == 'y') {
                ...
                for (size_t i = 0; i < foodMenu.size(); i++) {
                    cout << "\t\t" << i + 1 << ". " << foodMenu[i].name<<" gia " << foodMenu[i].price << " VND\n";
                }
            ...
            if (foodCost > 0) {
                cout << "\t\tChi phi cho an uong: " << foodCost << " VND\n";
            }
            cout << "\t\tTong chi phi: " << totalCost << " VND\n";
            pressEnterToContinue();
        }

        void cancelRent() { // hàm hủy vé
            system("cls");
            displayHeader();
            string ticketCode;
            ...
            RentalRecord cancelRecord = *it;
            cancelRecord.action = "CANCEL";
            rentalHistory.push_back(cancelRecord);

            boat->cancelSeats(cancelRecord.shiftIndex, cancelRecord.seatsRented);
            saveRentalHistoryToFile();
            saveBoatsToFile();
        ...
        void loadRentalHistoryFromFile() {  // hàm quan trọng, giúp hoạt động kể cả khi tắt và khởi động lại ctrinh
